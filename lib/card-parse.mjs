...
export function pickPrice(text) {
  const t = String(text ?? '');
  const NUM = '(\\d{1,3}(?:\\.\\d{3})+|\\d{4,9})';
  const forms = [
    { re: new RegExp(`${NUM}\\s*(?:TL|₺)`, 'g'), numGroup: 1 },   // 1.250.000 TL
    { re: new RegExp(`(?:₺|TL)\\s*${NUM}`, 'g'), numGroup: 1 },   // ₺ 1.378.750
  ];
  const found = [];
  for (const { re, numGroup } of forms) {
    ...
    while ((m = re.exec(t))) {
      ...
      const value = Number(m[numGroup].replace(/\./g, ''));
...
export function pickKm(text) {
  const m = String(text ?? '').match(/(\d{1,3}(?:\.\d{3})+|\d{1,7})\s*KM/i);
  if (!m) return null;
...
export function parseCardText(text, { source, city = null } = {}) {
  ...
  const segVariant = variantSeg ? variantSeg.replace(/^(SUV|Sedan|Hatchback|Crossover|Station Wagon|Coupe|MPV|Pickup)\s+/i, '').trim() : null;
