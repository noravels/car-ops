...
/** WebSocket el sıkışması denemesi (Chrome HTTP uçları kapalıyken tek yol). */
...
export async function cdpAvailable(endpoint = DEFAULT_ENDPOINT) {
  ...
  for (const url of candidateWsUrls(endpoint)) {
    ...
    if (attempt.ok) {
      ...
      return { ok: true, transport: 'ws', browser: 'Chrome (WS üzerinden)', wsUrl: url };
    }
    errors.push(`${url}: ${attempt.error}`);
  }
  return { ok: false, error: errors.join(' | ') || 'CDP ucu bulunamadı' };
}
